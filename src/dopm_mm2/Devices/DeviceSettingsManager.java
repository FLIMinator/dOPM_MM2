...
public class DeviceSettingsManager {
    ...
    private void initVars(){
        ...
        try { xyStageName = core_.getXYStageDevice(); } catch(Exception e) { xyStageName = ""; }
        ...
        try { zStageName = core_.getFocusDevice(); } catch(Exception e) { zStageName = ""; }
        ...
        imageView1 = true;
        imageView2 = false;
        opmAngle = 35; // 45 deg vs 35 deg
        magnification = 6.5/(1.333*(50/(180/50))); // 20*1.4*200/180; Leo silicon to Hugh water dOPM system 6.5 um cam pick
    ...
    /** NEW: Deep log of current hardware state to ensure traceability. */
    public void logSystemState() {
        deviceManagerLogger.info("--- OPM HARDWARE SNAPSHOT ---");
        deviceManagerLogger.info("Camera: " + dOPMCameraName);
        try {
			deviceManagerLogger.info("Live ScanMode: " + core_.getProperty(dOPMCameraName, "ScanMode")); // NEW
    ...
    public double getCameraReadoutTime(double exposureMs) throws Exception{
        ...
        switch (trigger){
            case 0: 
                readout_time = (Vn + Math.ceil(exp2Ms/oneHMs) + 4) * oneHMs;
                break;
            case 1:
                readout_time = ((Vn+1)*oneHMs + Math.max(0, (exposureMs - minReadoutTimeMs))); 
                break;
            case 2:
    ...
    private boolean checkInDeviceList(List<String> deviceNames){
        if (deviceNames == null) return false;
    ...
    private double getMaxExposureInAcq() throws Exception{
        ...
        for (ChannelSpec cs : acqChannels){ 
            if(cs.exposure() > maxExposure) maxExposure = cs.exposure(); 
        }
        return maxExposure;
    }

    /** REVISION: MDA-Aware. Uses MDA exposure settings if configured, otherwise falls back to Core Live exposure. */
    public void updateMaxTriggeredXyScanSpeed(){
        try { 
            ...
            try { maxTriggeredXyScanSpeed = calculateMaxTriggeredXyScanSpeed(core_.getExposure()); } catch(Exception ignored) {}
        }
    }
    
    public double calculateMaxTriggeredXyScanSpeed(double exp){
        try { return (getXyStageTriggerDistance() / getCameraReadoutTime(exp)) * getScanSpeedSafetyFactorXy(); }
        catch (Exception e){ return xyStageGlobalScanSpeed; }
    }
    ...
    public void updateMaxTriggeredMirrorScanSpeed(){
        try { 
            ...
            maxTriggeredMirrorScanSpeed = calculateMaxTriggeredMirrorScanSpeed(targetExp); 
             ...
             try { maxTriggeredMirrorScanSpeed = calculateMaxTriggeredMirrorScanSpeed(core_.getExposure()); } catch(Exception ignored) {}
        }
    }
    
    public double calculateMaxTriggeredMirrorScanSpeed(double exp){
        try { return (getMirrorTriggerDistance() / getCameraReadoutTime(exp)) * getScanSpeedSafetyFactorMirror(); }
        catch (Exception e){ return mirrorStageGlobalScanSpeed; }
    }
    
    public void updateMaxGlobalTriggeredXyScanSpeed(){
        try { maxGlobalTriggeredXyScanSpeed = calculateMaxTriggeredXyScanSpeed(getMaxExposureInAcq()); } catch(Exception e) {}
    }
    
    public void upateMaxGlobalTriggeredMirrorScanSpeed(){
        try { maxGlobalTriggeredMirrorScanSpeed = calculateMaxTriggeredMirrorScanSpeed(getMaxExposureInAcq()); } catch(Exception e) {}
    }

    public double getScanSpeedSafetyFactorMirror() { return scanSpeedSafetyFactorMirror; }
    public void setScanSpeedSafetyFactorMirror(double f) { this.scanSpeedSafetyFactorMirror = f; updateMaxTriggeredMirrorScanSpeed(); }
    public double getScanSpeedSafetyFactorXy() { return scanSpeedSafetyFactorXy; }
    public void setScanSpeedSafetyFactorXy(double f) { this.scanSpeedSafetyFactorXy = f; updateMaxTriggeredXyScanSpeed(); }
    ...
    public void setTriggerMode(int m) { this.triggerMode = m; updateMaxTriggeredMirrorScanSpeed(); }
    ...
    public void setExposureTime(double e) { this.exposureTime = e; updateMaxTriggeredMirrorScanSpeed(); updateMaxTriggeredXyScanSpeed(); }
    ...
    public void setFrameSize(Rectangle r) { this.frameSize = r; updateMaxTriggeredXyScanSpeed(); updateMaxTriggeredMirrorScanSpeed(); }
    ...
    public String getProperty(String device, String propety){
        String value = "";
        try{ value = core_.getProperty(device, propety); } catch (Exception e){ deviceManagerLogger.severe(e.getMessage()); }
        return value;
    }
    
    public void setProperty(String device, String propety, String propertyValue){
        try{ core_.setProperty(device, propety, propertyValue); } catch (Exception e){ deviceManagerLogger.severe(e.getMessage()); }
    }
